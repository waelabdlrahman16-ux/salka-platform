...
// Matches the feed_ads_link_shape check constraint. Duplicated here on
// purpose: the database is the one that decides, this only means the admin
// is told what is wrong while typing rather than by a raw Postgres error.
const LINK_OK = (v: string) => !v.trim() || /^\/[A-Za-z0-9/_?=&%.:-]*$/.test(v) || /^https?:\/\/\S+$/.test(v)

// 2 MB and this list are enforced on the bucket too. Checking here as well
// only saves the upload round trip; it is not the guard.
const MAX_BYTES = 2 * 1024 * 1024
const OK_TYPES = ['image/jpeg', 'image/png', 'image/webp', 'image/avif']

/**
 * Ad blocks placed BETWEEN restaurant cards on Home -- a second, separate
 * rotation from BannersAdmin's rail at the top of the page. Same shape and
 * the same admin workflow on purpose (an operator who already knows one
...
export default function FeedAdsAdmin({ restaurants }: { restaurants: Restaurant[] }) {
  ...
  const [editing, setEditing] = useState<FeedAdRow | 'new' | null>(null)
  ...
  async function upload(file: File) {
    ...
    // Own sub-path in the shared `banners` bucket -- same permission model,
    // no reason to stand up a second bucket for the same kind of asset.
    const path = `feed-ads/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${ext}`
  ...
  async function save() {
    ...
    const res = editing === 'new'
      ? await supabase.from('feed_ads').insert({ ...payload, sort: rows.length + 1 })
      : await supabase.from('feed_ads').update(payload).eq('id', (editing as FeedAdRow).id)
    setSaving(false)
  ...
  async function toggle(r: FeedAdRow) {
    setError('')
    const { error } = await supabase.from('feed_ads').update({ active: !r.active }).eq('id', r.id)
    if (error) { setError(describeError(error.message)); return }
    load()
  }

  async function move(r: FeedAdRow, dir: -1 | 1) {
    const i = rows.findIndex(x => x.id === r.id)
    const j = i + dir
    if (j < 0 || j >= rows.length) return
    setError('')
    const a = rows[i], b = rows[j]
    const r1 = await supabase.from('feed_ads').update({ sort: b.sort }).eq('id', a.id)
    if (r1.error) { setError(describeError(r1.error.message)); return }
    const r2 = await supabase.from('feed_ads').update({ sort: a.sort }).eq('id', b.id)
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-mist">بتظهر بين المطاعم في الصفحة الرئيسية، مش فوق زي الإعلانات التانية.</p>
