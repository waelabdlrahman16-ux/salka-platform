...
export default function AddonLibrary({ restaurantId, items }: {
  ...
  async function remove(id: number) {
    if (!confirm('حذف من المكتبة؟ الأصناف اللي اتضاف لها قبل كده مش هتتأثر.')) return
  ...
  async function applyToCategory() {
    ...
    setNotice(n === 0
      ? `"${applying.name}" كان موجود بالفعل على كل أصناف ${targetCat || 'القايمة'} — مفيش حاجة اتغيرت`
      : `اتضاف "${applying.name}" لـ ${n} صنف. تقدر تعدّل سعره على كل صنف لوحده من شاشة الصنف.`)
    setApplying(null)
  }
  ...
  return (
    <div className="card p-4 mb-4">
      ...
      <p className="text-xs text-mist mb-3">
        اكتب الإضافة مرة واحدة (اسم + صورة + سعر)، وبعدين ضيفها لقسم كامل بضغطة —
        زي "طماطم" على كل الساندوتشات. بعد كده تقدر تغيّر سعرها على أي صنف لوحده.
      </p>
      ...
      {applying && (
        <div className="bg-shellup/60 rounded-xl p-3 mb-3">
          <p className="text-xs font-semibold mb-2">ضيف "{applying.name}" لكل أصناف قسم:</p>
          ...
          <label className="label !text-xs">تحت أي مجموعة عند العميل</label>
          <input className="field !py-1.5 text-sm mb-2" value={groupName} onChange={e => setGroupName(e.target.value)}
            placeholder="إضافات" />
          <p className="text-[11px] text-mist mb-2">
            الأصناف اللي فيها الإضافة دي بالفعل مش هتتكرر. السعر هنا مجرد بداية — عدّله على كل صنف لوحده بعد كده.
          </p>
      ...
      <div className="bg-shellup/60 rounded-xl p-3">
        <p className="text-xs font-semibold mb-2">إضافة جديدة للمكتبة</p>
        ...
        <div className="flex gap-2">
          <input className="field !py-1.5 text-sm" placeholder="اسم الإضافة (مثال: طماطم)" value={draft.name}
