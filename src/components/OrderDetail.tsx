...
export function OrderLines({ order }: { order: Order }) {
  ...
  const extras = (
    <>
      {note && <p className="text-xs text-coral-700 mt-2 pt-2 border-t border-dashed border-line"><Icon name="penToSquare" size="xs" className="inline-block align-[-0.15em] me-1" />{note}</p>}
      {order.prescription_path && (
        <p className="text-xs text-coral-700 mt-1"><Icon name="paperclip" size="xs" className="inline-block align-[-0.15em] me-1" />العميل رفع صورة روشتة. شوفها من شاشة الإدارة</p>
      )}
    </>
  )
