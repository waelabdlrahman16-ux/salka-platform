...
// The six top-level groups all carry a marker, so they all get an icon. The
// SUB-tabs below them are the opposite case: only 6 of 22 ever had an emoji,
// which made the marker the anomaly rather than the pattern -- so those are
// now text-only throughout. Consistent either way, just in opposite directions.
const GROUPS: { key: TabGroup; label: string; icon: IconName }[] = [
  { key: 'now',     label: 'التشغيل',   icon: 'trafficSignal' },
  { key: 'money',   label: 'الفلوس',    icon: 'coins' },
  { key: 'catalog', label: 'المطاعم',   icon: 'forkKnife' },
  { key: 'places',  label: 'الأماكن',   icon: 'locationDot' },
  { key: 'people',  label: 'الناس',     icon: 'users' },
  { key: 'setup',   label: 'الإعدادات', icon: 'gear' },
]
...
const TABS: { key: Tab; label: string; group: TabGroup }[] = [
  ...
  { key: 'promos', label: 'أكواد الخصم', group: 'money' },
  ...
  async function runLoad() {
    try {
      ...
      const [
        ...
        withTimeout(supabase.from('orders').select('*, restaurants(name)')
          .eq('refund_status', 'pending').is('archived_at', null).order('id', { ascending: false })),
        withTimeout(supabase.from('orders').select('*, restaurants(name)')
          .is('archived_at', null)
        ...
        <div className="space-y-6">
          ...
          {(() => {
            ...
            const filteredOrders = source.filter(o => {
              // Archived records remain auditable in the database but stay out of
              // everyday operational/history views.
              if ((o as Order & { archived_at?: string | null }).archived_at) return false
