-- track_order() explicitly lists which order columns to return, and
-- promo_discount was never in that list -- the order-summary/tracking
-- screen never even received the discount amount from the API, regardless
-- of anything on the frontend. Adds promo_discount plus the code text
-- (joined from promo_codes) so the customer can see what was applied, not
