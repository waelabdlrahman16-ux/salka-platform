-- Supervisors can continue to quote, prepare, and dispatch orders, but only an
-- administrator may cancel a staff-owned order. Customer and vendor cancellation
...
-- The no-answer endpoint can cancel/refund an order as one of its resolutions.
-- Leave supervisors able to record wait/failed delivery, but reserve those two
-- cancellation outcomes for administrators as well.
create or replace function public.admin_resolve_no_answer(
...
begin
  ...
  if p_action in ('refund', 'cancel') and not is_admin() then
    raise exception 'admin_only';
  end if;
  perform private.admin_resolve_no_answer(p_assignment_id, p_action);
end $function$;
