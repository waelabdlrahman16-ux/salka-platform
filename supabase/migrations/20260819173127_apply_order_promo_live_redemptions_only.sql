...
begin
  ...
  -- released_at is null: a cancelled order returns its redemption to the pool.
  -- Without this the code burned through max_redemptions on orders that never
  -- happened -- 6 of the first 13 on SOKHNA30.
  ...
  -- ONE identity per human: the phone. This used to be 'customer:<id>' when
  -- signed in and 'phone:<number>' when not, so the same person got a fresh
  -- allowance by signing in between two orders -- and a phone with two customer
  -- rows got one each. The phone is the thing that does not change.
